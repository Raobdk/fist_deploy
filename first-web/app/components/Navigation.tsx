...
export default function Navigation() {
  ...
  return (
    <>
      {/* Main Navigation */}
      <nav className={`fixed top-4 left-4 right-4 md:top-8 md:left-1/2 md:transform md:-translate-x-1/2 z-50 transition-all duration-500 ${isScrolled ? 'bg-black/80 backdrop-blur-md border border-amber-400/20' : 'bg-transparent'} rounded-full px-4 md:px-8 py-3 md:py-4`}>
        <div className="flex items-center justify-between md:justify-center md:space-x-8">
          {/* Logo */}
          <Link href="/" className="flex items-center space-x-2 md:space-x-3">
            <img 
              src="https://static.readdy.ai/image/2bf0fcc592d2aaefc450fb1148f0185f/b793749e574a9093f2d220a0cfe4ae67.png" 
              alt="Talent Desk" 
              className="w-6 h-6 md:w-8 md:h-8 object-contain"
            />
            <span className="font-pacifico text-lg md:text-xl text-amber-400">Talent Desk</span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-6">
            {navItems.map((item) => (
              <Link
                ...
                className={`group relative px-4 py-2 text-sm font-medium transition-all duration-300 cursor-pointer whitespace-nowrap ${activeSection === item.id ? 'text-amber-400' : 'text-gray-300 hover:text-white'}`}
              >
            ...
            <div className="flex flex-col space-y-2">
              {navItems.map((item) => (
                <Link
                  ...
                  href={item.href}
                  onClick={() => scrollToSection(item.id)}
                  className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-300 cursor-pointer ${activeSection === item.id ? 'bg-amber-400/20 text-amber-400' : 'text-gray-300 hover:bg-gray-700/50 hover:text-white'}`}
                >
              ...
              <button
                ...
                className="flex items-center space-x-3 px-4 py-3 bg-gradient-to-r from-amber-400 to-yellow-600 text-black font-medium rounded-lg mt-4 cursor-pointer whitespace-nowrap"
              >
      ...
      <div className="hidden md:flex fixed right-8 top-1/2 transform -translate-y-1/2 z-40 flex-col space-y-4">
        {[ 
          ...
          { icon: 'ri-mail-line', label: 'Email', action: () => window.open('mailto:contact@wealthdesign.com') },
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="relative w-full max-w-2xl bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl border border-amber-400/20 p-6 md:p-8">
            ...
            <div className="text-center mb-6 md:mb-8">
              ...
              <p className="text-gray-300 text-sm md:text-base">Book your personalized wealth management consultation</p>
            </div>
